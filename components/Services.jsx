...
export default function Services() {
  return (
    <section id="services" className="px-6 sm:px-8 lg:px-12 py-28 bg-surface">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {services.map((service, index) => (
            <ScrollReveal key={service.title} delay={0.1 + index * 0.12} scale className="h-full">
              <SpotlightCard className="card-hover bg-base border border-line rounded-2xl p-8 hover:border-accent/30 h-full">
                <div className="relative z-10">
                  ...
                  <div className="w-11 h-11 rounded-xl bg-surface border border-line flex items-center justify-center mb-6 text-accent group-hover:bg-accent group-hover:text-white group-hover:border-accent transition-all duration-300">
                    {service.icon}
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {service.tags.map((tag) => (
                      <span key={tag} className="text-xs font-medium text-white/25 bg-white/5 border border-line px-3 py-1 rounded-full group-hover:text-white/40 group-hover:border-accent/20 transition-all duration-300">
                        {tag}
                      </span>
                    ))}
                  </div>
